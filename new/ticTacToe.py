...
def minimax(board: List[List[str]], K: int, depth: int, maximizing: bool) -> Tuple[int, Optional[Tuple[int,int]]]:
    ...
    if maximizing:
        ...
        for (r, c) in moves:
            board[r][c] = AI
            val, _ = minimax(board, K, depth-1, False)
            board[r][c] = EMPTY
            if val > best_val:
        ...
        for (r, c) in moves:
            board[r][c] = HUMAN
            val, _ = minimax(board, K, depth-1, True)
            board[r][c] = EMPTY
            if val < best_val:
                best_val, best_move = val, (r, c)
        return best_val, best_move
...
def ai_move(board: List[List[str]], K: int, depth: int) -> Tuple[int,int]:
    if len(available_moves(board)) == len(board) * len(board):
        n = len(board)
        return n//2, n//2
    _, move = minimax(board, K, depth, True)
    if move is None:
        move = random.choice(available_moves(board))
    return move

def play():
    print("=== N x N Tic-Tac-Toe (with AI, Minimax only) ===")
